...
void SetMenuActive(bool enabled) { menuActive = enabled; }
...
void UpdateMenu() {
  ...
  if (!menuActive) {
    // If we request to close the menu, and game isn't loaded, force the menu
    // open.
  ...
  if (IsLibretroReady()) {
    const char *statusText =
        TextFormat("%s %s", GetLibretroName(), GetLibretroVersion());
    Rectangle statusBarRect =
        (Rectangle){0, GetScreenHeight() - 28, GetScreenWidth(), 28};
    GuiStatusBar(statusBarRect, statusText);
  } else {
    GuiLabel((Rectangle){padding + buttonWidth, padding,
                         GetScreenWidth() - buttonWidth, buttonHeight},
             " ...or Drag and Drop.");
    GuiDrawText("raylib-libretro",
  ...
  if (!openFileDialog.windowActive) {
    ...
    Rectangle closeButton =
        (Rectangle){GetScreenWidth() - buttonHeight - padding, padding,
    ...
    Rectangle fullscreenCheckbox = (Rectangle){
        GetScreenWidth() - buttonHeight * 2 - padding - smallPadding, padding,
        buttonHeight, buttonHeight};
    ...
    // Volume
    Rectangle muteCheckbox = (Rectangle){GetScreenWidth() - buttonHeight * 4 -
                                             (padding - smallPadding) * 2,
                                         padding, buttonHeight, buttonHeight};
    int muteIcon = (GetLibretroCore()->volume >= 0.5f) ? ICON_AUDIO : ICON_WAVE;
    if (GuiButton(muteCheckbox, GuiIconText(muteIcon, ""))) {
      if (GetLibretroCore()->volume <= 0.5f) {
        GetLibretroCore()->volume = 1.0f;
      } else {
        GetLibretroCore()->volume = 0.f;
      }
    }
    ...
    if (IsLibretroReady()) {
      ...
      if (GuiButton((Rectangle){padding, padding, buttonWidth, buttonHeight},
                    GuiIconText(ICON_FOLDER_FILE_OPEN, "Open Game"))) {
      ...
      // Run/Close Game
      Rectangle runResetGameRect =
          (Rectangle){padding, buttonHeight * 2 + padding + smallPadding * 2,
                      buttonWidth, buttonHeight};
      if (!IsLibretroGameReady()) {
        if (!DoesLibretroCoreNeedContent()) {
          // GuiSetTooltip("Run the core without content");
          if (GuiButton(runResetGameRect,
                        GuiIconText(ICON_ARROW_RIGHT_FILL, "Run"))) {
        ...
        if (GuiButton(runResetGameRect,
                      GuiIconText(ICON_CROSS, "Close Game"))) {
          UnloadLibretroGame();
        }
        // GuiClearTooltip();
      }
      ...
      if (IsLibretroGameReady()) {
        ...
        if (GuiButton((Rectangle){padding,
                                  buttonHeight * 3 + padding + smallPadding * 3,
                                  buttonWidth, buttonHeight},
                      GuiIconText(ICON_ROTATE_FILL, "Reset"))) {
          ResetLibretro();
          menuActive = false;
        }
        // GuiClearTooltip();
  ...
  if (openFileDialog.SelectFilePressed) {
    ...
    switch (openFileType) {
    case 0:
      TraceLog(LOG_INFO, "LIBRETRO: Open Core %s/%s",
               openFileDialog.dirPathText, openFileDialog.fileNameText);
      InitLibretro(TextFormat("%s/%s", openFileDialog.dirPathText,
                              openFileDialog.fileNameText));
      break;
    case 1:
      TraceLog(LOG_INFO, "LIBRETRO: Open Content %s/%s",
               openFileDialog.dirPathText, openFileDialog.fileNameText);
      if (LoadLibretroGame(TextFormat("%s/%s", openFileDialog.dirPathText,
                                      openFileDialog.fileNameText))) {
        menuActive = false;
      }
      break;

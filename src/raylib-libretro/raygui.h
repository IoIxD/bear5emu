 ...
 *
 *   DESCRIPTION:
 *       raygui is a tools-dev-focused immediate-mode-gui library based on
 *raylib but also available as a standalone library, as long as input and
 *drawing functions are provided.
 *
 *   FEATURES:
 ...
 *       - No auto-layout mechanism, up to the user to define controls position
 *and size
 *       - Standalone mode requires library modification and some user work to
 *plug another backend
 *
 *   NOTES:
 *       - WARNING: GuiLoadStyle() and GuiLoadStyle{Custom}() functions,
 *allocate memory for font atlas recs and glyphs, freeing that memory is
 *(usually) up to the user, no unload function is explicitly provided... but
 *note that GuiLoadStyleDefaulf() unloads by default any previously loaded font
 *(texture, recs, glyphs).
 *       - Global UI alpha (guiAlpha) is applied inside GuiDrawRectangle() and
 *GuiDrawText() functions
 *
 *   CONTROLS PROVIDED:
 ...
 *       raygui uses a global data array for all gui style properties (allocated
 *on data segment by default), when a new style is loaded, it is loaded over the
 ...
 *default one
 *
 *       The global style array size is fixed and depends on the number of
 *controls and properties:
 *
 *           static unsigned int
 *guiStyle[RAYGUI_MAX_CONTROLS*(RAYGUI_MAX_PROPS_BASE +
 ...
 *
 *       Custom control properties can be defined using the EXTENDED properties
 *for each independent control.
 *
 *       TOOL: rGuiStyler is a visual tool to customize raygui style:
 *github.com/raysan5/rguistyler
 *
 *
 *   RAYGUI ICONS (guiIcons):
 *       raygui could use a global array containing icons data (allocated on
 *data segment by default), a custom icons set could be loaded over this array
 *using GuiLoadIcons(), but loaded icons set must be same RAYGUI_ICON_SIZE and
 *no more than RAYGUI_ICON_MAX_ICONS will be loaded
 *
 ...
 *
 *       When the icon is draw, actually one quad per pixel is drawn if the bit
 *for that pixel is set.
 *
 *       The global icons array size is fixed and depends on the number of icons
 *and size:
 *
 *           static unsigned int
 *guiIcons[RAYGUI_ICON_MAX_ICONS*RAYGUI_ICON_DATA_ELEMENTS];
 *
 *       guiIcons size is by default: 256*(16*16/32) = 2048*4 = 8192 bytes = 8
 *KB
 *
 *       TOOL: rGuiIcons is a visual tool to customize/create raygui icons:
 *github.com/raysan5/rguiicons
 *
 *   RAYGUI LAYOUT:
 *       raygui currently does not provide an auto-layout mechanism like other
 *libraries, layouts must be defined manually on controls drawing, providing the
 *right bounds Rectangle for it.
 *
 *       TOOL: rGuiLayout is a visual tool to create raygui layouts:
 *github.com/raysan5/rguilayout
 *
 *   CONFIGURATION:
 ...
 *           Avoid including embedded ricons data (256 icons, 16x16 pixels,
 *1-bit per pixel, 2KB)
 *
 ...
 *                         ADDED: guiControlExclusiveMode and
 *guiControlExclusiveRec for exclusive modes
 *
 *       4.0 (12-Sep-2023) ADDED: GuiToggleSlider()
 *                         ADDED: GuiColorPickerHSV() and GuiColorPanelHSV()
 *                         ADDED: Multiple new icons, mostly compiler related
 *                         ADDED: New DEFAULT properties: TEXT_LINE_SPACING,
 *TEXT_ALIGNMENT_VERTICAL, TEXT_WRAP_MODE ADDED: New enum values:
 *GuiTextAlignment, GuiTextAlignmentVertical, GuiTextWrapMode ADDED: Support
 *loading styles with custom font charset from external file REDESIGNED:
 *GuiTextBox(), support mouse cursor positioning REDESIGNED: GuiDrawText(),
 *support multiline and word-wrap modes (read only) REDESIGNED: GuiProgressBar()
 *to be more visual, progress affects border color REDESIGNED: Global alpha
 *consideration moved to GuiDrawRectangle() and GuiDrawText() REDESIGNED:
 *GuiScrollPanel(), get parameters by reference and return result value
 *                         REDESIGNED: GuiToggleGroup(), get parameters by
 *reference and return result value REDESIGNED: GuiComboBox(), get parameters by
 *reference and return result value REDESIGNED: GuiCheckBox(), get parameters by
 *reference and return result value REDESIGNED: GuiSlider(), get parameters by
 *reference and return result value REDESIGNED: GuiSliderBar(), get parameters
 *by reference and return result value REDESIGNED: GuiProgressBar(), get
 *parameters by reference and return result value REDESIGNED: GuiListView(), get
 *parameters by reference and return result value REDESIGNED: GuiColorPicker(),
 *get parameters by reference and return result value REDESIGNED:
 *GuiColorPanel(), get parameters by reference and return result value
 *                         REDESIGNED: GuiColorBarAlpha(), get parameters by
 *reference and return result value REDESIGNED: GuiColorBarHue(), get parameters
 *by reference and return result value REDESIGNED: GuiGrid(), get parameters by
 *reference and return result value REDESIGNED: GuiGrid(), added extra parameter
 *                         REDESIGNED: GuiListViewEx(), change parameters order
 *                         REDESIGNED: All controls return result as int value
 *                         REVIEWED: GuiScrollPanel() to avoid smallish
 *scroll-bars REVIEWED: All examples and specially controls_test_suite RENAMED:
 *gui_file_dialog module to gui_window_file_dialog UPDATED: All styles to
 *include ISO-8859-15 charset (as much as possible)
 *
 ...
 *                         REVIEWED: Local GetCodepointNext(), aligned with
 *raylib REDESIGNED: GuiSlider*()/GuiScrollBar() to support out-of-bounds
 *
 *       3.5 (20-Apr-2023) ADDED: GuiTabBar(), based on GuiToggle()
 *                         ADDED: Helper functions to split text in separate
 *lines ADDED: Multiple new icons, useful for code editing tools REMOVED:
 *Unneeded icon editing functions REMOVED: GuiTextBoxMulti(), very limited and
 *broken REMOVED: MeasureTextEx() dependency, logic directly implemented
 *                         REMOVED: DrawTextEx() dependency, logic directly
 *implemented REVIEWED: GuiScrollBar(), improve mouse-click behaviour REVIEWED:
 *Library header info, more info, better organized REDESIGNED: GuiTextBox() to
 *support cursor movement REDESIGNED: GuiDrawText() to divide drawing by lines
 *
 *       3.2 (22-May-2022) RENAMED: Some enum values, for unification, avoiding
 *prefixes REMOVED: GuiScrollBar(), only internal REDESIGNED: GuiPanel() to
 *support text parameter REDESIGNED: GuiScrollPanel() to support text parameter
 *                         REDESIGNED: GuiColorPicker() to support text
 *parameter REDESIGNED: GuiColorPanel() to support text parameter REDESIGNED:
 *GuiColorBarAlpha() to support text parameter REDESIGNED: GuiColorBarHue() to
 *support text parameter REDESIGNED: GuiTextInputBox() to support password
 *
 *       3.1 (12-Jan-2022) REVIEWED: Default style for consistency (aligned with
 ...
 *                         REDESIGNED: GuiListView*(), GuiDropdownBox(),
 *GuiSlider*(), GuiProgressBar(), GuiMessageBox() REVIEWED: GuiTextBox(),
 *GuiSpinner(), GuiValueBox(), GuiLoadStyle() Replaced property INNER_PADDING by
 *TEXT_PADDING, renamed some properties ADDED: 8 new custom styles ready to use
 *                         Multiple minor tweaks and bugs corrected
 *
 *       2.5 (28-May-2019) Implemented extended GuiTextBox(), GuiValueBox(),
 *GuiSpinner() 2.3 (29-Apr-2019) ADDED: rIcons auxiliar library and support for
 *it, multiple controls reviewed Refactor all controls drawing mechanism to use
 *control state 2.2 (05-Feb-2019) ADDED: GuiScrollBar(), GuiScrollPanel(),
 *reviewed GuiListView(), removed Gui*Ex() controls 2.1 (26-Dec-2018)
 *REDESIGNED: GuiCheckBox(), GuiComboBox(), GuiDropdownBox(), GuiToggleGroup() >
 *Use combined text string REDESIGNED: Style system (breaking change) 2.0
 *(08-Nov-2018) ADDED: Support controls guiLock and custom fonts REVIEWED:
 *GuiComboBox(), GuiListView()... 1.9 (09-Oct-2018) REVIEWED: GuiGrid(),
 *GuiTextBox(), GuiTextBoxMulti(), GuiValueBox()... 1.8 (01-May-2018) Lot of
 *rework and redesign to align with rGuiStyler and rGuiLayout 1.5 (21-Jun-2017)
 *Working in an improved styles system 1.4 (15-Jun-2017) Rewritten all GUI
 *functions (removed useless ones) 1.3 (12-Jun-2017) Complete redesign of style
 *system 1.1 (01-Jun-2017) Complete review of the library 1.0 (07-Jun-2016)
 *Converted to header-only by Ramon Santamaria. 0.9 (07-Mar-2016) Reviewed and
 *tested by Albert Martos, Ian Eito, Sergio Martinez and Ramon Santamaria. 0.8
 *(27-Aug-2015) Initial release. Implemented by Kevin Gato, Daniel Nicolás and
 *Ramon Santamaria.
 *
 *   DEPENDENCIES:
 *       raylib 5.0  - Inputs reading (keyboard/mouse), shapes drawing, font
 *loading and text drawing
 *
 *   STANDALONE MODE:
 *       By default raygui depends on raylib mostly for the inputs and the
 *drawing functionality but that dependency can be disabled with the config flag
 *RAYGUI_STANDALONE. In that case is up to the user to provide another backend
 *to cover library needs.
 *
 ...
 *           - int GetCharPressed(void);         // -- GuiTextBox(),
 *GuiValueBox()
 *
 *           - void DrawRectangle(int x, int y, int width, int height, Color
 *color); // -- GuiDrawRectangle()
 *           - void DrawRectangleGradientEx(Rectangle rec, Color col1, Color
 *col2, Color col3, Color col4); // -- GuiColorPicker()
 *
 *           - Font GetFontDefault(void);                            // --
 *GuiLoadStyleDefault()
 *           - Font LoadFontEx(const char *fileName, int fontSize, int
 **codepoints, int codepointCount); // -- GuiLoadStyle()
 ...
 *           - unsigned char *DecompressData(const unsigned char *compData, int
 *compDataSize, int *dataSize); // -- GuiLoadStyle()
 *
 *   CONTRIBUTORS:
...
typedef enum {
  ...
  // GLOBAL for all controls TEXT_SPACING,               // Control text spacing
  // between glyphs -> GLOBAL for all controls TEXT_LINE_SPACING           //
  // Control text spacing between lines -> GLOBAL for all controls
  ...
  // GLOBAL for all controls
} GuiControlProperty;

// TODO: Which text styling properties should be global or per-control?
// At this moment TEXT_PADDING and TEXT_ALIGNMENT is configured and saved per
// control while TEXT_SIZE, TEXT_SPACING, TEXT_LINE_SPACING,
// TEXT_ALIGNMENT_VERTICAL, TEXT_WRAP_MODE are global and should be configured
// by user as needed while defining the UI layout
...
// WARNING: We only have 8 slots for those properties by default!!! -> New
// global control: TEXT?
typedef enum {
  ...
  TEXT_ALIGNMENT_VERTICAL, // Text vertical alignment inside text bounds (after
                           // border and padding)
  TEXT_WRAP_MODE           // Text wrap-mode inside text bounds
  // TEXT_DECORATION             // Text decoration: 0-None, 1-Underline,
  // 2-Line-through, 3-Overline TEXT_DECORATION_THICK       // Text decoration
  // line thikness
} GuiDefaultProperty;

// Other possible text properties:
// TEXT_WEIGHT                  // Normal, Italic, Bold -> Requires specific
// font change TEXT_INDENT                  // Text indentation -> Now using
// TEXT_PADDING...
...
typedef enum {
  TEXT_READONLY =
      16, // TextBox in read-only mode: 0-text editable, 1-text no-editable
} GuiTextBoxProperty;
...
typedef enum {
  ...
  SCROLLBAR_SIDE,         // ListView scrollbar side (0-SCROLLBAR_LEFT_SIDE,
                          // 1-SCROLLBAR_RIGHT_SIDE)
} GuiListViewProperty;
...
RAYGUIAPI void GuiSetAlpha(float alpha); // Set gui controls alpha (global
...
RAYGUIAPI void
GuiLoadStyleDefault(void); // Load style default over global style
...
RAYGUIAPI char **
GuiLoadIcons(const char *fileName,
             bool loadIconsName); // Load raygui icons file (.rgi) into internal
                                  // icons data
RAYGUIAPI void
GuiDrawIcon(int iconId, int posX, int posY, int pixelSize,
            Color color); // Draw icon using pixel size at specified position
#endif

// Controls
//----------------------------------------------------------------------------------------------------------
// Container/separator controls, useful for controls organization
RAYGUIAPI int GuiWindowBox(
    Rectangle bounds,
    const char *title); // Window Box control, shows a window that can be closed
RAYGUIAPI int GuiGroupBox(Rectangle bounds,
                          const char *text); // Group Box control with text name
RAYGUIAPI int
GuiLine(Rectangle bounds,
        const char *text); // Line separator control, could contain text
RAYGUIAPI int
GuiPanel(Rectangle bounds,
         const char *text); // Panel control, useful to group controls
RAYGUIAPI int
GuiTabBar(Rectangle bounds, const char **text, int count,
          int *active); // Tab Bar control, returns TAB to be closed or -1
RAYGUIAPI int GuiScrollPanel(Rectangle bounds, const char *text,
                             Rectangle content, Vector2 *scroll,
                             Rectangle *view); // Scroll Panel control

// Basic controls set
RAYGUIAPI int GuiLabel(Rectangle bounds, const char *text); // Label control
RAYGUIAPI int
GuiButton(Rectangle bounds,
          const char *text); // Button control, returns true when clicked
RAYGUIAPI int GuiLabelButton(
    Rectangle bounds,
    const char *text); // Label button control, returns true when clicked
RAYGUIAPI int GuiToggle(Rectangle bounds, const char *text,
                        bool *active); // Toggle Button control
RAYGUIAPI int GuiToggleGroup(Rectangle bounds, const char *text,
                             int *active); // Toggle Group control
RAYGUIAPI int GuiToggleSlider(Rectangle bounds, const char *text,
                              int *active); // Toggle Slider control
RAYGUIAPI int
GuiCheckBox(Rectangle bounds, const char *text,
            bool *checked); // Check Box control, returns true when active
RAYGUIAPI int GuiComboBox(Rectangle bounds, const char *text,
                          int *active); // Combo Box control

RAYGUIAPI int GuiDropdownBox(Rectangle bounds, const char *text, int *active,
                             bool editMode); // Dropdown Box control
RAYGUIAPI int GuiSpinner(Rectangle bounds, const char *text, int *value,
                         int minValue, int maxValue,
                         bool editMode); // Spinner control
RAYGUIAPI int GuiValueBox(
...
RAYGUIAPI int GuiTextBox(Rectangle bounds, char *text, int textSize,
                         bool editMode); // Text Box control, updates input text

RAYGUIAPI int GuiSlider(Rectangle bounds, const char *textLeft,
                        const char *textRight, float *value, float minValue,
                        float maxValue); // Slider control
RAYGUIAPI int GuiSliderBar(Rectangle bounds, const char *textLeft,
                           const char *textRight, float *value, float minValue,
                           float maxValue); // Slider Bar control
RAYGUIAPI int GuiProgressBar(Rectangle bounds, const char *textLeft,
                             const char *textRight, float *value,
                             float minValue,
                             float maxValue); // Progress Bar control
RAYGUIAPI int
GuiStatusBar(Rectangle bounds,
             const char *text); // Status Bar control, shows info text
RAYGUIAPI int GuiDummyRec(Rectangle bounds,
                          const char *text); // Dummy control for placeholders
RAYGUIAPI int GuiGrid(Rectangle bounds, const char *text, float spacing,
                      int subdivs, Vector2 *mouseCell); // Grid control

// Advance controls set
RAYGUIAPI int GuiListView(Rectangle bounds, const char *text, int *scrollIndex,
                          int *active); // List View control
...
RAYGUIAPI int
GuiMessageBox(Rectangle bounds, const char *title, const char *message,
              const char *buttons); // Message Box control, displays a message
RAYGUIAPI int
GuiTextInputBox(Rectangle bounds, const char *title, const char *message,
                const char *buttons, char *text, int textMaxSize,
                bool *secretViewActive); // Text Input Box control, ask for
                                         // text, supports secret
RAYGUIAPI int
GuiColorPicker(Rectangle bounds, const char *text,
               Color *color); // Color Picker control (multiple color controls)
RAYGUIAPI int GuiColorPanel(Rectangle bounds, const char *text,
                            Color *color); // Color Panel control
RAYGUIAPI int GuiColorBarAlpha(Rectangle bounds, const char *text,
                               float *alpha); // Color Bar Alpha control
RAYGUIAPI int GuiColorBarHue(Rectangle bounds, const char *text,
                             float *value); // Color Bar Hue control
RAYGUIAPI int GuiColorPickerHSV(
    Rectangle bounds, const char *text,
    Vector3 *colorHsv); // Color Picker control that avoids conversion to RGB on
                        // each call (multiple color controls)
RAYGUIAPI int GuiColorPanelHSV(
    Rectangle bounds, const char *text,
    Vector3 *colorHsv); // Color Panel control that updates Hue-Saturation-Value
                        // color value, used by GuiColorPickerHSV()
//----------------------------------------------------------------------------------------------------------
